...
class GraphicsArenaViewer : public GraphicsApp {
 ...
 private:
   ...
   * @param[in] robot The robot handle.
   */
  //void DrawRobotSensors(NVGcontext *ctx, const class Robot* const robot);

  /**
   * @brief Draw a circular in the arena using nanogui.
   *
   * This function requires an active nanovg drawing context (ctx), so it should
   * probably only be called from with \ref DrawUsingNanoVG().
   *
   ...
   */
  void DrawCircularObstacle(NVGcontext *ctx,
                            const class CircularObstacle* const obstacle);

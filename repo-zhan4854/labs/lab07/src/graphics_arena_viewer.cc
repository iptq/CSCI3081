...
void GraphicsArenaViewer::OnSpecialKeyDown(int key, int scancode, int modifiers) {
  EventKeypress e(key);
  arena_->Accept(&e);
...
void GraphicsArenaViewer::DrawCircularObstacle(NVGcontext *ctx,
                                       const CircularObstacle* const obstacle) {
  nvgBeginPath(ctx);
  nvgCircle(ctx, obstacle->get_pos().x(), obstacle->get_pos().y(), obstacle->radius());
...
void GraphicsArenaViewer::DrawUsingNanoVG(NVGcontext *ctx) {
  ...
  std::vector<CircularObstacle*> obstacles = arena_->obstacles();
  for (size_t i = 0; i < obstacles.size(); i++) {
    DrawCircularObstacle(ctx, obstacles[i]);
  } /* for(i..) */

...
Arena::Arena(const struct arena_params* const params) :
  ...
  for (size_t i = 0; i < n_obstacles_; ++i) {
    entities_.push_back(new CircularObstacle(
...
std::vector<CircularObstacle*> Arena::obstacles(void) {
  std::vector<CircularObstacle*> res;
  for (auto ent : entities_) {
    CircularObstacle* o = dynamic_cast<CircularObstacle*>(ent);
    if (o) {
      res.push_back(o);
    }
  } /* for(ent..) */
...
void Arena::UpdateEntitiesTimestep(void) {
  ...
  for (auto ent : mobile_entities_) {
    ...
    // If not at wall, check if it is colliding with any other entities (not itself)
    if (!ec.collided()) {

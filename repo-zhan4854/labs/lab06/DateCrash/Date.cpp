...
Date::Date() {
    if (DEBUG) cout << "... calling default constructor for Date..." << endl;
    year = 1900;
    month = 1;
    day = 1;
}
...
Date::~Date() {
    if (DEBUG)
        cout << "... calling destructor for Date \"" << show() << "\" ..."
             << endl;
}
...
bool Date::equals(const Date &d) const {
    assert(d.year >= 1900) assert(0 <= d.month && d.month <= 12)
...
bool is_valid_date(int y, int m, int d) {
    ...
    if (y >= 1900 && m >= 1 && m <= 12 && d >= 1 && d <= month_length[m]) {
        return true;
    } else {
        return false;
    }
...
Date mkDate(int y, int m, int d) {
    if (DEBUG) cout << "... calling mkDate to create Date ..." << endl;

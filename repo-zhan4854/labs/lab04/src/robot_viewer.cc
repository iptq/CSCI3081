...
RobotViewer::RobotViewer()
    ...
    pause_btn_ = gui->addButton(
        "Pause", std::bind(&RobotViewer::OnPauseBtnPressed, this));
    gui->addButton("Restart",
                   std::bind(&RobotViewer::OnRestartBtnPressed, this));
    gui->addButton("Change Color",
...
void RobotViewer::OnColorChangeBtnPressed() { robot_land_->changeColors(); }
...
void RobotViewer::DrawRobot(NVGcontext *ctx, double sim_time, int id,
                            Robot *robot) {
    // void RobotViewer::DrawRobot(NVGcontext *ctx, int id, double xpos, double
    // ypos,
    ...
    double xpos, ypos;
    robot->get_pos(sim_time, &xpos, &ypos);

    double xvel, yvel;
    robot->get_vel(sim_time, &xvel, &yvel);

    double rad = robot->get_radius();
...
void RobotViewer::DrawUsingNanoVG(NVGcontext *ctx) {
    ...
    double sim_time = robot_land_->get_current_time();
    ...
    for (int i = 0; i < robot_land_->get_num_robots(); i++) {
        ...
        */
        ...
        DrawRobot(ctx, sim_time, i, robot_land_->get_robot(i));

...
int RobotLand::get_num_robots(void) { return this->robots.size(); }

void RobotLand::changeColors() {
    for (int i = 0, l = this->robots.size(); i < l; ++i) {
        this->robots[i]->changeColors();
    }
}

Robot *RobotLand::get_robot(int id) { return this->robots[id]; }
...
bool RobotLand::get_robot_pos(int id, double *x_pos, double *y_pos) {
    if (id >= this->get_num_robots()) return false;
    Robot *robot = this->robots[id];
    return robot->get_pos(sim_time_, x_pos, y_pos);
...
bool RobotLand::get_robot_vel(int id, double *x_vel, double *y_vel) {
    if (id >= this->get_num_robots()) return false;
    Robot *robot = this->robots[id];
    return robot->get_vel(sim_time_, x_vel, y_vel);
    /*
